...
Shader "Hidden/StableFluids"
{
    ...
    half4 frag_advect(v2f_img i) : SV_Target
    {
        ...
        float2 aspect = float2(_MainTex_TexelSize.y * _MainTex_TexelSize.z, 2);
        float2 aspect_inv = float2(_MainTex_TexelSize.x * _MainTex_TexelSize.w, 2);
        ...
        float3 dye = saturate(sin(0.5 * float3(2.72, 5.12, 4.98)) + 0.5);
